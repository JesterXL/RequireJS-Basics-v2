        ...
        load: function (name, parentRequire, load, config) {
          //>>excludeStart('excludeHbs', pragmas.excludeHbs)
            ...
            if (disableI18n){
              ...
              var localeDirectory = (config.hbs && config.hbs.i18nDirectory ? config.hbs.i18nDirectory : i18nDirectory);
              var localeJSON =  (config.locale || "en_us") + '.json';
              var parentRequireURL = parentRequire.toUrl(localeDirectory + localeJSON);
                 
                fetchOrGetCached(parentRequireURL,function (langMap) {
                  var langJSON;
                  try
                  {
                    langJSON = JSON.parse(langMap);
                  }
                  catch(e)
                  {
                    logger.error("Language Locale JSON Parsing error:", e);
                    return;
                  }
                  fetchAndRegister(langJSON);
                });
            }
          //>>excludeEnd('excludeHbs')
        }
